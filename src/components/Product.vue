<template lang="pug">
    v-layout(row wrap justify-center)#resume
        ...
        v-flex(md10 xs12).my-3
            ...
            v-layout(row wrap)
                v-flex(v-if="repos.length > 0" v-for="(r, idx) in repos" :key="`github-repo-${idx}`" xs12)
                ...
                v-flex(v-else xs12).text-xs-center: v-progress-circular(indeterminate color="grey")
        ...
        v-flex(md10 xs12).my-3
            ...
            v-layout(row wrap)
                v-flex(v-if="articles.length > 0" v-for="(a, idx) in articles" :key="`qiita-article-${idx}`" xs12)
