...
export class MobileComponent {
  ...
  constructor() {
    ...
    this.breadcrumbs = document.querySelector(
      '.breadcrumbs'
    ) as HTMLInputElement;
  ...
  private viewChanger(main: HTMLInputElement, state: string): void {
    ...
    switch (state) {
      case 'left':
        this.viewIdx =
          this.viewIdx >= this.views.length - 1 ? 0 : this.viewIdx + 1;
        break;
      case 'right':
        this.viewIdx =
          this.viewIdx <= 0 ? this.views.length - 1 : this.viewIdx - 1;
        break;
    }
  ...
  private setBreadcrumb(view: HTMLInputElement): void {
    ...
    // const breadcrumb = this.breadcrumbs?.querySelector(
    //   `span[data-target="${pageId}"]`
    // ) as HTMLInputElement;
