...
$(function(){$('.modal').modal();controlPagePosition();});function controlPagePosition(){var pageTopBtn=$('#page-top');var height=150;if($(this).scrollTop()<height){pageTopBtn.hide();}
...
cour=4;var $ul=document.querySelector('#anime-list');var fragment=document.createDocumentFragment();$.getJSON('https://api.moemoe.tokyo/anime/v1/master/'+year+'/'+cour).done(function(data){for(var i=0;i<data.length;i++){var $li=document.createElement('li');$li.innerHTML=data[i].title;fragment.appendChild($li);}
$ul.appendChild(fragment);}).fail(function(data){var $li=document.createElement('li');$li.innerHTML='データの取得に失敗しました．通信環境を確認してください．';console.error(data.statusText);});}
